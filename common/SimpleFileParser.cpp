...
SimpleFileParser::SimpleFileParser(const SimpleFileParser& obj)
{
}

SimpleFileParser& SimpleFileParser::operator=(const SimpleFileParser& obj)
{
  return *this;
}
...
bool SimpleFileParser::file_exists(const char* fn)
{
  ifstream ifs(fn);
  if (!ifs || ifs.bad())
    return false;
...
bool SimpleFileParser::file_exists(const string& fn) {
  return file_exists(fn.c_str());
}

bool SimpleFileParser::open(const string& fn)
{
  if (!file_exists(fn))
    return false;
  close();
...
bool SimpleFileParser::eof() const
{
  return m_ifs ? m_ifs->eof() : true;
}

void SimpleFileParser::rewind()
{
  if (m_ifs) {
    m_ifs->seekg(0);
    m_lineno = 0;
...
void SimpleFileParser::set_single_char_tokens(const char* tokens)
{
  size_t sz = sizeof(m_single_char_tokens)/sizeof(bool);
  for(size_t i=0;i<sz;++i)
    m_single_char_tokens[i] = false;
...
const char* SimpleFileParser::get_single_char_tokens() const
{
...
char* SimpleFileParser::skip_to_end_of_comment(char* p)
{
  for(;*p;++p) {
    if (*p == '*' && p[1] == '/') {
      p++;
...
char* SimpleFileParser::skip_comments(char* p)
{
  if (!*p) {
    return p;
  }
...
char* SimpleFileParser::skip_whitespace(char* p)
{
  if (!*p)
    return p;
  for(;*p!=0 && (*p<=' ' || *p>=127);++p);
...
char* SimpleFileParser::skip_to_whitespace(char* p)
{
  for(;*p!=0 && *p>' ' && *p<127;++p);
  return p;
}

char* SimpleFileParser::is_continuation_line(char* line) const
{
  char* p = line;
...
void SimpleFileParser::add_token(char* token)
{
...
size_t SimpleFileParser::get_next_line()
{
...
bool SimpleFileParser::get_next_line(size_t num_tokens,
                                       const string& tok1,
                                       const string& tok2,
                                       const string& tok3,
                                       const string& tok4,
...
bool SimpleFileParser::is_token_uint(size_t i) const
{
...
bool SimpleFileParser::is_token_int(size_t i) const
{
  const char* p = m_tokens[i].c_str();
